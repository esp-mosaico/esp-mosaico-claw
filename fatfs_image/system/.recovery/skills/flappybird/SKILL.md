...
- An LCD registered as `display_lcd` in `board_manager`.
- Touch registered as `lcd_touch` is the default. Pass `input_mode="button"`
  and a physical button on `pin` / `button_gpio` to use GPIO input instead.
...
Pass an empty `args` object for defaults (display touch, 16 kHz requested audio
sample rate, `run_time_ms=180000`).

Common optional args:

| Arg | Default | Meaning |
|-----|---------|---------|
| `input_mode` | `touch` | Use `touch` or `button` input |
...
runtime stops it. On startup or runtime failure, report the `[flappybird] ...`
