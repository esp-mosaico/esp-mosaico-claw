...
#define CAP_SYSTEM_PLATFORM_INFO_BUFFER_SIZE    2048
...
static EXT_RAM_BSS_ATTR cap_system_platform_provider_t s_platform_provider;
...
{
    if (!provider) {
        memset(&s_platform_provider, 0, sizeof(s_platform_provider));
        return ESP_OK;
    }
    if (!provider->get_info && !provider->execute) {
...
static esp_err_t cap_system_add_platform_section(cJSON *root, const char *name);
...
static esp_err_t cap_system_render_json(cJSON *root, char *output, size_t output_size)
{
    ...
    written = snprintf(output, output_size, "%s", rendered);
    free(rendered);
    if (written < 0 || (size_t)written >= output_size) {
        output[0] = '\0';
        ESP_LOGE(TAG, "render json: output buffer too small");
        return ESP_ERR_INVALID_SIZE;
    }
    return ESP_OK;
}
...
static cJSON *cap_system_build_info_json(void)
{
    ...
    (void)cap_system_add_platform_section(root, "identity");
    (void)cap_system_add_platform_section(root, "battery");
    (void)cap_system_add_platform_section(root, "display");
    (void)cap_system_add_platform_section(root, "audio");
    (void)cap_system_add_platform_section(root, "network");
    (void)cap_system_add_platform_section(root, "power");
    return root;
}
...
static esp_err_t cap_system_add_platform_section(cJSON *root, const char *name)
{
    ...
    if (!s_platform_provider.get_info) {
        section = cJSON_CreateObject();
        if (!section) {
            return ESP_ERR_NO_MEM;
        }
        cJSON_AddBoolToObject(section, "supported", false);
        cJSON_AddStringToObject(section, "message", "platform provider is not configured");
        cJSON_AddItemToObject(root, name, section);
        return ESP_OK;
    }

    section_json = calloc(1, CAP_SYSTEM_PLATFORM_INFO_BUFFER_SIZE);
    if (!section_json) {
        return ESP_ERR_NO_MEM;
    }
    err = s_platform_provider.get_info(name,
                                       section_json,
                                       CAP_SYSTEM_PLATFORM_INFO_BUFFER_SIZE,
                                       s_platform_provider.user_ctx);
    if (section_json[0]) {
        section = cJSON_Parse(section_json);
    }
    free(section_json);
    ...
    if (!section) {
        ...
        cJSON_AddBoolToObject(section, "supported", false);
        cJSON_AddStringToObject(section, "error", esp_err_to_name(err));
    }
    cJSON_AddItemToObject(root, name, section);
    return ESP_OK;
}
...
static esp_err_t cap_system_build_selected_info_json(const cJSON *sections, char *output, size_t output_size)
{
    ...
    if (cap_system_section_is_requested(sections, "identity")) {
        ESP_GOTO_ON_ERROR(cap_system_add_platform_section(root, "identity"), cleanup, TAG, "Failed to add identity section");
    }
    if (cap_system_section_is_requested(sections, "battery")) {
        ESP_GOTO_ON_ERROR(cap_system_add_platform_section(root, "battery"), cleanup, TAG, "Failed to add battery section");
    }
    if (cap_system_section_is_requested(sections, "display")) {
        ESP_GOTO_ON_ERROR(cap_system_add_platform_section(root, "display"), cleanup, TAG, "Failed to add display section");
    }
    if (cap_system_section_is_requested(sections, "audio")) {
        ESP_GOTO_ON_ERROR(cap_system_add_platform_section(root, "audio"), cleanup, TAG, "Failed to add audio section");
    }
    if (cap_system_section_is_requested(sections, "network")) {
        ESP_GOTO_ON_ERROR(cap_system_add_platform_section(root, "network"), cleanup, TAG, "Failed to add network section");
    }
    if (cap_system_section_is_requested(sections, "power")) {
        ESP_GOTO_ON_ERROR(cap_system_add_platform_section(root, "power"), cleanup, TAG, "Failed to add power section");
    }
...
static esp_err_t cap_system_execute_platform(const char *operation,
                                             const char *input_json,
                                             char *output,
                                             size_t output_size)
{
    if (!operation || !output || output_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    if (!s_platform_provider.execute) {
        snprintf(output,
                 output_size,
                 "{\"ok\":false,\"supported\":false,\"error\":\"platform provider is not configured\"}");
        return ESP_ERR_NOT_SUPPORTED;
    }

    return s_platform_provider.execute(operation,
...
static esp_err_t cap_system_parse_operation(const char *input_json,
                                            cJSON **ret_input,
                                            const char **ret_operation,
                                            char *output,
                                            size_t output_size)
{
    cJSON *input;
    ...
    if (!ret_input || !ret_operation || !output || output_size == 0) {
        return ESP_ERR_INVALID_ARG;
    }
    *ret_input = NULL;
    *ret_operation = NULL;
    ...
    operation = cJSON_GetObjectItemCaseSensitive(input, "operation");
    if (!cJSON_IsString(operation) || !operation->valuestring || !operation->valuestring[0]) {
        cJSON_Delete(input);
        snprintf(output, output_size, "{\"ok\":false,\"error\":\"operation is required\"}");
        return ESP_ERR_INVALID_ARG;
    }

    *ret_input = input;
    *ret_operation = operation->valuestring;
    return ESP_OK;
}

static esp_err_t cap_system_report_unknown_operation(const char *operation,
                                                     char *output,
                                                     size_t output_size)
{
    snprintf(output,
             output_size,
             "{\"ok\":false,\"error\":\"unsupported operation\",\"operation\":\"%s\"}",
             operation ? operation : "");
    return ESP_ERR_NOT_SUPPORTED;
}

static esp_err_t cap_system_execute_query(const char *input_json,
                                          const claw_cap_call_context_t *ctx,
                                          char *output,
                                          size_t output_size)
{
    ...
    esp_err_t err = cap_system_parse_operation(input_json,
                                               &input,
                                               &operation,
                                               output,
                                               output_size);
    ...
    if (strcmp(operation, "get_system_info") == 0) {
        err = cap_system_execute_get_info(input_json, ctx, output, output_size);
    } else if (strcmp(operation, "get_current_time") == 0) {
        err = cap_system_execute_get_current_time(input_json, ctx, output, output_size);
    } else if (strcmp(operation, "scan_wifi_networks") == 0) {
        err = cap_system_execute_platform(operation, input_json, output, output_size);
    } else {
        err = cap_system_report_unknown_operation(operation, output, output_size);
    }
...
static esp_err_t cap_system_execute_control(const char *input_json,
                                            const claw_cap_call_context_t *ctx,
                                            char *output,
                                            size_t output_size)
{
    ...
    (void)ctx;
    err = cap_system_parse_operation(input_json, &input, &operation, output, output_size);
    if (err != ESP_OK) {
        return err;
    }

    if (strcmp(operation, "set_display") == 0 ||
            strcmp(operation, "set_audio") == 0 ||
            strcmp(operation, "vibrate") == 0 ||
            strcmp(operation, "set_power_config") == 0) {
        err = cap_system_execute_platform(operation, input_json, output, output_size);
    } else {
        err = cap_system_report_unknown_operation(operation, output, output_size);
    }
...
static esp_err_t cap_system_execute_admin(const char *input_json,
                                          const claw_cap_call_context_t *ctx,
                                          char *output,
                                          size_t output_size)
{
    ...
    if (strcmp(operation, "configure_wifi") == 0 ||
            strcmp(operation, "enter_sleep") == 0) {
        err = cap_system_execute_platform(operation, input_json, output, output_size);
    } else if (strcmp(operation, "restart_device") == 0) {
        err = cap_system_execute_restart(input_json, ctx, output, output_size);
    } else {
        err = cap_system_report_unknown_operation(operation, output, output_size);
    }
...
static const claw_cap_descriptor_t s_system_descriptors[] = {
    {
        ...
        .name = "system_query",
        .family = "system",
        .description = "Query device state. Use get_system_info with optional sections; get_current_time for local time; or "
                       "scan_wifi_networks with optional limit. Return only available data; never fabricate values.",
        .kind = CLAW_CAP_KIND_CALLABLE,
        .cap_flags = CLAW_CAP_FLAG_CALLABLE_BY_LLM,
        .input_schema_json = "{\"type\":\"object\",\"properties\":{"
                             "\"operation\":{\"type\":\"string\",\"enum\":[\"get_system_info\",\"get_current_time\",\"scan_wifi_networks\"]},"
                             "\"sections\":{\"type\":\"array\",\"items\":{\"type\":\"string\",\"enum\":[\"chip\",\"uptime\",\"version\","
                             "\"memory\",\"cpu\",\"wifi\",\"ip\",\"identity\",\"battery\",\"display\",\"audio\",\"network\",\"power\"]},"
                             "\"minItems\":1,\"uniqueItems\":true},"
                             "\"limit\":{\"type\":\"integer\",\"minimum\":1,\"maximum\":30}},"
                             "\"required\":[\"operation\"],\"additionalProperties\":false}",
        .execute = cap_system_execute_query,
    },
    {
        .id = "system_control",
        .name = "system_control",
        .family = "system",
        .description = "Control normal device functions using operation: set_display with brightness and/or rotation; set_audio with "
                       "volume and/or muted; vibrate with duration_ms; set_power_config with profile.",
        .kind = CLAW_CAP_KIND_CALLABLE,
        .cap_flags = CLAW_CAP_FLAG_CALLABLE_BY_LLM,
        .input_schema_json = "{\"type\":\"object\",\"properties\":{"
                             "\"operation\":{\"type\":\"string\",\"enum\":[\"set_display\",\"set_audio\",\"vibrate\",\"set_power_config\"]},"
                             "\"brightness\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":100},"
                             "\"rotation\":{\"type\":\"integer\",\"enum\":[0,90,180,270]},"
                             "\"volume\":{\"type\":\"integer\",\"minimum\":0,\"maximum\":100},"
                             "\"muted\":{\"type\":\"boolean\"},"
                             "\"duration_ms\":{\"type\":\"integer\",\"minimum\":10,\"maximum\":5000},"
                             "\"profile\":{\"type\":\"string\",\"enum\":[\"performance\",\"balanced\",\"low_power\"]}},"
                             "\"required\":[\"operation\"],\"additionalProperties\":false}",
        .execute = cap_system_execute_control,
    },
    {
        .id = "system_admin",
        .name = "system_admin",
        .family = "system",
        .description = "Perform root-only system administration using operation: configure_wifi requires ssid and password; enter_sleep "
                       "requires mode and duration_ms; restart_device accepts optional delay_ms.",
        .kind = CLAW_CAP_KIND_CALLABLE,
        .cap_flags = CLAW_CAP_FLAG_CALLABLE_BY_LLM | CLAW_CAP_FLAG_ROOT_AGENT_ONLY,
        .input_schema_json = "{\"type\":\"object\",\"properties\":{"
                             "\"operation\":{\"type\":\"string\",\"enum\":[\"configure_wifi\",\"enter_sleep\",\"restart_device\"]},"
                             ...
                             "\"mode\":{\"type\":\"string\",\"enum\":[\"light\",\"deep\"]},"
                             "\"duration_ms\":{\"type\":\"integer\",\"minimum\":1000,\"maximum\":86400000},"
                             "\"delay_ms\":{\"type\":\"integer\",\"minimum\":0}},"
                             "\"required\":[\"operation\"],\"additionalProperties\":false}",
        .execute = cap_system_execute_admin,
    },
};
